...
# The single CAgg added for the editor-version drill-down (migration 0050),
# created WITH NO DATA — its own 7-day refresh policy only materializes
# going forward from whenever it first fires, so without this one-time
# backfill it'd sit empty until the policy slowly caught up on its own.
EDITOR_VERSION_CAGG_NAMES = ['cagg_editor_version_daily']

# Matches the existing single-dimension CAggs' own coverage start (see
# docs/todo/cagg-history-coverage-gap.md for why they stop there rather
# than covering full 2005+ history).
DEFAULT_START = datetime(2025, 8, 1, tzinfo=dt_timezone.utc)


class Command(BaseCommand):
    help = (
        "One-time backfill for cagg_editor_version_daily (migration 0050). Idempotent — safe to "
        "re-run or interrupt and resume (refresh_continuous_aggregate over an already-current "
        "range is a cheap no-op). Mirrors backfill_country_caggs.py's structure exactly: that "
        "command's own docstring documents this exact kind of backfill crashing Postgres "
        "repeatedly on this host even with max_parallel_workers_per_gather=0 as a role default — "
        "small/paced batches and a crash/cooldown/retry loop are load-bearing here, not optional "
        "tuning, even though cagg_editor_version_daily's (editor, version) cardinality is "
        "expected to be far smaller than the country pairs'. This command walks the range one "
        "batch at a time *itself* (not via refresh_caggs_over_range's own internal loop) "
        "specifically so it can track the resume point as plain Python state, advanced only "
        "after a batch actually commits — re-deriving the resume point from max(bucket) after a "
        "crash would silently produce a false-positive 'complete' the first time this runs, since "
        "the CAgg's own automatic 7-day refresh policy keeps materializing the last few days "
        "regardless of this backfill's progress."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help='Days per refresh call (default: 2 — kept small given this host'
    ...
    def handle(self, *args, **options):
        ...
        while cursor_start < end:
            ...
            try:
                # See db/init/02-role-statement-timeout.sh — a dense
                # batch's refresh can legitimately run past the role
                # default, so opt out. Must be re-issued on every attempt,
                # not just once at startup: a crash closes this connection,
                # and the fresh reconnect after that would otherwise
                # silently pick the role default back up.
                connection.cursor().execute("SET statement_timeout = 0")
                refresh_caggs_over_range(
                    cursor_start, batch_end, cagg_names=EDITOR_VERSION_CAGG_NAMES, batch_days=batch_days,
                    stdout=self.stdout,
                )
        ...
        self.stdout.write(self.style.SUCCESS(
            f'Editor-version CAgg backfill complete: {overall_start.date()}..{end.date()} in '
