...
# The 3 cross-dimension CAggs added in migration 0041, created WITH NO DATA
...
PAIR_CAGG_NAMES = ['cagg_editor_imagery_daily', 'cagg_editor_locale_daily', 'cagg_imagery_locale_daily']
...
# these three are meant to serve the same "past year" window ToplistView/
...
class Command(BaseCommand):
    help = (
        "One-time backfill for the 3 cross-dimension CAggs added in migration 0041 "
        "(cagg_editor_imagery_daily, cagg_editor_locale_daily, cagg_imagery_locale_daily). "
        "Idempotent — safe to re-run or interrupt and resume (refresh_continuous_aggregate "
        "over an already-current range is a cheap no-op). Batch size defaults smaller than "
        "the usual 30 days (see CLAUDE.md's statement_timeout notes on this host's I/O "
        "constraints) — pass --batch-days to widen it once this has been proven safe."
    )
